 ...
 *
 * @Block(
 *   id = "attendee_registration_confirm",
 *   admin_label = @Translation("Attendee registration confirmation"),
 *   provider = "node",
 *   category = @Translation("Forms")
 * )
 */
class AttendeeRegistrationConfirmBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $conf_id = $session->get('registration_id');
    $current_user = \Drupal::currentUser();

    $entity_attendee = \Drupal::entityTypeManager()->getStorage('attendee_registration')->load((int) $conf_id);
    if ($entity_attendee && count($entity_attendee) >= 1) {
      //$entity_attendee = end($entity_attendee);
    }
    if ($entity_attendee->field_ar_registration_confirmed->value == '1') {
      $config = $config = \Drupal::config('ila_attendee_registration_block.settings');
      return ['#markup' => t($config->get('message.register'))];
    }

    $build['form'] = \Drupal::formBuilder()->getForm('Drupal\ila_attendee_registration_block\Plugin\Form\AttendeeRegistrationConfirmForm');
